...
PCA9685Node::PCA9685Node()
{
    ...
    dc_sub_ = nh_.subscribe<pca9685_board::pwm>(
        "pwm_msg", 1000, &PCA9685Node::pwm_callback_, this);
}
 ...
 */
void PCA9685Node::pwm_callback_(
    const pca9685_board::pwmConstPtr& msg
)
{
...
void PCA9685Node::stop_(const int channel)
{
    float curr_val = board_controller_.curr_pwm[channel];
    int step = int((curr_val - MIN_RUN_PWM) / 10);
    for (int i; i<10; i++)
        board_controller_.set_pwm(channel, curr_val - step * (i + 1));
}
